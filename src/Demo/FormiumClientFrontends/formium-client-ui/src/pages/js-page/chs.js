  ...
  return (
    <article>
      ...
      <div className="section-block">
        <p>
          NanUI 提供与 CEF 的 JavaScript 环境交互的能力。您可以在 .NET 环境中同步或异步执行 JavaScript 代码并获得返回值。
        </p>
        <p>
          <b>执行无返回值 JavaScript 代码</b>
        </p>
        <p>
          在 NanUI 的浏览器环境初始化之后就可以使用 <b>ExecuteJavaScript</b> 方法来执行 JavaScript 代码。
        </p>
        ...
        <p>
          在 NanUI 的浏览器环境初始化之后就可以使用 <b>EvaluateJavaScriptAsync</b> 方法来异步执行带返回值的 JavaScript
          代码。
        </p>
        <div className="alert alert-light">
          <pre>
...
return { 
    ...
    var f = await obj["f"].ExecuteAsync(fobj);
        ...
        <p>您可以在示例源码 MainWindow.cs 的 TestEvaluateJavaScriptAsync 方法中设置上断点，单步调试方法内的代码来查看每一步代码执行的详情。</p>
        <p>
          <button
            className="btn btn-outline-secondary"
      ...
      <div className="section-block">
        <p>
          使用 <b>RegisterJavaScriptObject</b> 方法并配合 <b>JavaScriptValue</b> 对象可将 .NET 的对象、属性或方法等注册到 JavaScript 环境的 <b>Formium.external</b> 对象里。
        </p>
        <p><b>在 .NET 中注入对象</b></p>
        <div className="alert alert-light">
          <pre>{`private void TestJSObject()
{
    ...
    RegisterJavaScriptObject("tester", pObj);
        ...
        <p><b>在 JavaScript 中测试对象</b></p>
        <p>
          您现在可以从左侧打开<b>开发者工具</b>在控制台中输入下面代码观察注入的 .NET 对象，或者亲自执行一下来自 .NET 的各种方法。
        </p>
        <div className="alert alert-light">
          <pre>{`> Formium.external.tester`}</pre>
        </div>
        <p>需要特别指出的是，由于 CefGlue 的限制，在 .NET 中注册的异步方法实际在 JavaScript 环境中执行后并不直接返回 Promise 对象，而是返回了 Promise 对象的构造参数，因此您需要自行创建 Promise 对象。</p>
        <div className="alert alert-light">
      ...
      <div className="section-block">
        <p>除了使用上面的方式把 .NET 的对象注册到当前 JavaScript 环境中外，您还可以使用 JavaScript 窗体绑定技术来把您的 .NET 对象注册为全局的 JavaScript 对象。有关 CEF 中的 JavaScript 窗体绑定的知识，请查阅 CEF 官方文档《<a href="https://bitbucket.org/chromiumembedded/cef/wiki/JavaScriptIntegration" rel="noreferrer" target="_blank">JavaScriptIntegration</a>》。</p>
        <p>NanUI 简化了 CEF 的 JavaScript 窗体绑定过程。具体实现方式请查看本示例源代码<b>DemoWindowExtension\DemoWindow.cs</b>文件以及<b>Resources\DemoWindow.js</b>文件。DemoWindow.js 是 JavaScript 窗体绑定的核心，其中带有 native 关键字的 function 将自动对应到 DemoWindow.cs 中注册的 .NET 方法。您可以简单的理解为前端的 DemoWindow.js 文件既是后端的 DemoWindow.cs 的提纲，JavaScript 提纲中所提及（带有 native 关键字）的方法将自动绑定到 .NET 对应的方法上。</p>
        <p>使用 JavaScript 窗体绑定注册的对象将直接绑定到所有浏览器视图（包括 iframe）里。从左侧打开<b>开发者工具</b>在控制台中输入下面代码观察注入的 .NET 对象。</p>
        <div className="alert alert-light">
