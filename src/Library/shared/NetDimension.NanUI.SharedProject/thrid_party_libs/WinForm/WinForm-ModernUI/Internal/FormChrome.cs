    ...
    public abstract class FormChrome : Form
    {
        ...
        public FormChrome()
        {
            ...
            if (FormStyleHelper.IsWindows8OrLower)
            {
                ...
                primaryDpi = startupDpi = currentDpi = (int)dx;
            ...
            else
            {
                ...
                GetDpiForScreen(Screen.PrimaryScreen, DpiType.Effective, out var dpiX2, out var dpiY2);
                primaryDpi = (int)dpiX2;
            }
        ...
        private void HandleDpiChanged()
        {
            ...
            //var changedDpi = User32.GetOriginalDeviceDpi(Handle);

            //if (changedDpi == oldDpi) return;
        ...
        //RECT? wmDpiChangedSuggestedRect = null;

        protected virtual bool OnWMDpiChanged(ref Message m)
        {
            var dpiChangeDpi = (short)(int)m.WParam;
            ...
            if (oldDpi != currentDpi)
            {
                ...
                {
                    shouldScale = true;
                    User32.SetWindowPos(Handle, IntPtr.Zero, rect.Left, rect.Top, rect.Width, rect.Height, SetWindowPosFlags.SWP_NOACTIVATE | SetWindowPosFlags.SWP_NOZORDER | SetWindowPosFlags.SWP_NOSIZE);
                ...
                WriteConsoleLog($"[WM_DPICHANGED]: {oldDpi} -> {currentDpi} [SCALE_FACTOR]:{ (float)currentDpi / oldDpi } {rect}");
        ...
        private Padding ScalePadding(Padding value)
        {
            ...
            try
            {
                User32.GetDpiForMonitor(hMonitor, MonitorDpiType.MDT_EFFECTIVE_DPI, out dpi, out int dy);

                scaleFactor = (float)dpi / (float)designTimeDpi;
            ...
            var standardScale = (float)primaryDpi / (float)designTimeDpi;
            ...
            //MessageBox.Show($"scale:{scaleFactor} value:{value.Left}:{value.Top - value.Left} default:{standardBorder}:{standardCaption} translate:{border}:{caption}");

            //System.Diagnostics.Debug.WriteLine($"[PRI_SCR_DPI]:{primaryDpi} [START_SCR_DPI]:{startupDpi}");
            //System.Diagnostics.Debug.WriteLine($"[SYS_WIN]:{value.Left}:{value.Top - value.Bottom} [DEFAULT_WIN]:{standardBorder}:{standardCaption} [CALC_WIN]:{border}:{caption - border}");
        ...
        protected virtual void PatchClientSize()
        {
            ...
            var t = this.GetType();
            //var x = t.GetFields(BindingFlags.Instance | BindingFlags.NonPublic);

            FieldInfo fiWidth = t.GetField("clientWidth", BindingFlags.Instance | BindingFlags.NonPublic);
            FieldInfo fiHeight = t.GetField("clientHeight", BindingFlags.Instance | BindingFlags.NonPublic);
