    ...
    public class RestfulServiceProvider
    {
        ...
        public void ImportServiceAssembly(string fileName)
        {
            if (File.Exists(fileName))
            {
                ...
                {
                    NanUI.Log("[ResourceHandler:".Green(), $"{this.GetType().Name}".Cyan(), "]".Green(), " ", "[LOAD FAILED]:".Red(), $"{Path.GetFileName(fileName)}".Gray());
                    NanUI.Text($"{ex}");
                }
            }
            else
            {
                NanUI.Log("[ResourceHandler:".Green(), $"{this.GetType().Name}".Cyan(), "]".Green(), " ", "[NOT FOUND]:".Red(), $"{Path.GetFileName(fileName)}".Gray());
            }
