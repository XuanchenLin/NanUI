...
static class DpiHelper
{
    ...
    public static bool IsPerMonitorV2Awareness
    {
        get
        {
            ...
            {
                DPI_AWARENESS_CONTEXT? dpiAwareness = DpiMethods.TryGetThreadDpiAwarenessContext();
                var DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2 = new DPI_AWARENESS_CONTEXT(new IntPtr(-4));
    ...
    public static void InitializeDpiHelper()
    {
        ...
        if (Win32ApiHelper.IsApiAvailable("Shcore.dll", nameof(GetProcessDpiAwareness)))
        {
    ...
    public static int GetScreenDpiFromPoint(System.Drawing.Point point)
    {
        if (IsPerMonitorV2Awareness)
        {
    ...
    public static int GetScreenDpi(System.Windows.Forms.Screen currentScreen)
    {
        if (IsPerMonitorV2Awareness)
        {
            var hMonitor = MonitorFromPoint(new System.Drawing.Point(currentScreen.Bounds.X + currentScreen.Bounds.Width / 2, currentScreen.Bounds.Y + currentScreen.Bounds.Height / 2), MonitorFlags.MONITOR_DEFAULTTONEAREST);
    ...
    static class DpiMethods
    {
        ...
        {
            return Win32ApiHelper.IsApiAvailable("User32.dll", nameof(GetThreadDpiAwarenessContext));
        }

        public static bool SetThreadDpiAwarenessContextIsAvailable()
        {
            return Win32ApiHelper.IsApiAvailable("User32.dll", nameof(SetThreadDpiAwarenessContext));
        }

        public static bool AreDpiAwarenessContextsEqualIsAvailable()
        {
            return Win32ApiHelper.IsApiAvailable("User32.dll", nameof(AreDpiAwarenessContextsEqual));
        }

        public static bool GetWindowDpiAwarenessContextIsAvailable()
        {
            return Win32ApiHelper.IsApiAvailable("User32.dll", nameof(GetWindowDpiAwarenessContext));
        }
