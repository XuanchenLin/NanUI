...
public sealed class AppBuilder
{
    ...
    internal PlatformArchitecture Architecture
    {
        get
        {
            switch (IntPtr.Size)
            {
                ...
                default:
                    throw new NotSupportedException(NetDimension.NanUI.Properties.Messages.Exception_UnkownPlatformArchitecture);
            }
        }
    }
    ...
    public NanUIApp Build()
    {
        ...
        if (ProcessType == ProcessType.Main)
        {
            ...
            startup.ProgramMain(Environment.GetCommandLineArgs());
        ...
        {
            return new NanUIApp(ProcessType, Architecture, Services);
        });

        var app = Services.BuildServiceProvider().GetRequiredService<NanUIApp>();
