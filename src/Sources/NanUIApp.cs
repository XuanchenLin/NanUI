...
public sealed class NanUIApp
{
    private const string COMMON_DATA_DIR_NAME = @"NanUI";
    ...
    public static NanUIApp? Current { get; private set; }
    ...
    internal NanUILogger Logger => Services.GetRequiredService<NanUILogger>();
    ...
    internal NanUIApp(ProcessType processType, PlatformArchitecture architecture, IServiceCollection services, string? appName = null)
    {
        ...
        AppName = appName ?? Application.ProductName ?? "NanUI App";

        AppDataDirectory = DefaultAppDataDirectory;

        DefaultLogger defaultLogger = new DefaultLogger { Log = new NanUILogger(Path.Combine(AppDataDirectory, $"{nameof(NetDimension.NanUI).ToLower()}.log"), true) };


        services.AddSingleton(provider =>
        {
            return (NanUILogger)Logging.Logger.Instance.Log;
        });
    ...
    private CefSettings GetDefaultCefSettings()
    {
        return new CefSettings
        {
            ...
            LogFile = Path.Combine(AppDataDirectory, $"{nameof(NetDimension.NanUI).ToLower()}-cef.log"),
    ...
    private bool Load(CefMainArgs args, CefApp app, [Optional] out int exitCode)
    {
        ...
        var copyrightInfo = $"""
* Welcome to NanUI ({buildType}); Chromium Embedded/{CefRuntime.ChromeVersion}; NanUI/{Assembly.GetExecutingAssembly().GetName().Version}_{Architecture};
Copyrights (C) 2012-{DateTime.Now.Year} Xuanchen Lin all rights reserved. Made in Kunming, China.
{Resources.ASCII}
This project is under MIT License.

https://github.com/XuanchenLin/NanUI/blob/master/LICENCE
