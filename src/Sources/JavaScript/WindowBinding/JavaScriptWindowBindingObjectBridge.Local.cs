...
internal partial class JavaScriptWindowBindingObjectBridge
{
    ...
    private MessageBridgeResponse HandleExecuteWindowBindingObjectAsynchronousFunctionRequestOnLocal(MessageBridgeRequest request)
    {
        ...
        if (windowBindingObject == null)
        {
            ...
            {
                IsSuccess = false,
                Exception = $"[{nameof(NetDimension.NanUI)}]: The `{objectName}` window binding object is not exists."
            };
        }
        ...
        if (function == null)
        {
            ...
            {
                IsSuccess = false,
                Exception = $"[{nameof(NetDimension.NanUI)}]: The `{funcName}` function is not defined."
            };
        }
        ...
        if (function.FunctionType == JavaScriptWindowBindingObjectFunctionType.AsynchronousFunctionOnLocal)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    IsSuccess = false,
                    Exception = $"[{nameof(NetDimension.NanUI)}]: {ex.Message}"
                };
            }
        }
        ...
        {
            IsSuccess = false,
            Exception = $"[{nameof(NetDimension.NanUI)}]: The handler of `{funcName}` function is not defined."
        };
    }
