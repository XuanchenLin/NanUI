...
public abstract class JavaScriptWindowBindingObject : CefV8Handler
{
    ...
    protected override bool Execute(string name, CefV8Value obj, CefV8Value[] arguments, out CefV8Value returnValue, out string exception)
    {
        ...
        if (func == null)
        {
            exception = $"[{nameof(NetDimension.NanUI)}]: Native Function `{name}` is not defined.";
            returnValue = null;
            return true;
        }
            ...
            case JavaScriptWindowBindingObjectFunctionType.SynchronousFunctionOnRemote:
                {
                    ...
                    {
                        returnValue = null;
                        exception = $"[{nameof(NetDimension.NanUI)}]: Synchronous function `{name}` has no function handler.";
            ...
            case JavaScriptWindowBindingObjectFunctionType.AsynchronousFunctionOnRemote:
                {
                    ...
                    {
                        returnValue = null;
                        exception = $"[{nameof(NetDimension.NanUI)}]: Asynchronous function `{name}` has no function handler.";
