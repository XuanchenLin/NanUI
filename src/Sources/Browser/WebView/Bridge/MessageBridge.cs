...
internal sealed class MessageBridge : IDisposable
{
    ...
    public NanUIApp ApplicationContext { get; }
    ...
    internal static string GetPipeName(int browserId)
    {
        ...
        if (NanUIApp.Current!.ProcessType == ProcessType.Main)
        {
            processId = System.Diagnostics.Process.GetCurrentProcess().Id;
        }
        else
        {
            processId = NanUIApp.Current!.BrowserProcessId;
        }

        return $"NanUI-MessageBridgeProxy-{processId}-{browserId}";
    }

    public MessageBridge(NanUIApp app, CefBrowser browser, ProcessType side, ProcessMessageDispatcher messageDispatcher)
    {
        ApplicationContext = app;
        Browser = browser;
        Side = side;
        MessageDispatcher = messageDispatcher;
        MessageDispatcher.RegisterMessageHandler(NanUIMessage.WFM_MESSAGE_BRIDGE_MSG, OnMessageBridgeCommunicateCore);
    ...
    {
        var msg = CefProcessMessage.Create(NanUIMessage.WFM_MESSAGE_BRIDGE_MSG);
