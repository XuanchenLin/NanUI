...
namespace Chromium.WebBrowser
{
	...
	: this(parentContainer, null, true)
		...
		internal void OnBrowserCreated(CfxOnAfterCreatedEventArgs e)
		{
			...
			FormBorderStyle? storedFormBorderStyle = null;

			DisplayHandler.OnFullscreenModeChange += (that, args) =>
			{
				var fullscreen = args.Fullscreen;

				var form = this.parentControl.FindForm();


				form.RequireUIThread(() =>
				{
					if (fullscreen)
					{

						storedFormBorderStyle = form.FormBorderStyle;
					...
					{
						if (storedFormBorderStyle.HasValue)
						{
							form.FormBorderStyle = storedFormBorderStyle.Value;
						}
			...
			ContextMenuHandler.OnBeforeContextMenu += (that, args) =>
			{
		...
		internal static class CfxContextMenu
		{
			internal enum CfxContextMenuId
			{
				// Navigation.
				MENU_ID_BACK = 100,
				MENU_ID_FORWARD = 101,
			...
			public static bool IsEdition(CfxContextMenuId contextMenuId)
			{
			...
			{
				return (intValue >= (int)CfxContextMenuId.MENU_ID_UNDO) && (intValue <= (int)CfxContextMenuId.MENU_ID_SELECT_ALL);
			}

			public static bool IsUserDefined(int intValue)
			{
				return (intValue >= (int)CfxContextMenuId.MENU_ID_USER_FIRST) && (intValue <= (int)CfxContextMenuId.MENU_ID_USER_LAST);
			}
		}
